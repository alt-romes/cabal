...
addInternalBuildTools pkg lbi bi progs =
  (foldr updateProgram progs internalBuildTools)
    { progSearchPath = (map (ProgramSearchPathDir . takeDirectory . programPath) internalBuildTools) ++ progSearchPath progs
    , progOverrideEnv = pkgDataDirVar : progOverrideEnv progs
    }
  where
