  ...
  = do
      ...
      let customPp1Prog = simpleProgram "custom-pp1"
          ...
          customPp1 = case mbCustomPp1 of
            ...
            Nothing ->
              error $
                unlines
                  [ "package-with-build-tools: could not find custom-pp1 pre-processor in the program database."
          ...
          customPp2 = case mbCustomPp2 of
            ...
            Nothing ->
              error $
                unlines
                  [ "package-with-build-tools: could not find custom-pp2 pre-processor in the program database."
                  , "Component: " ++ show compNm ]
    ...
    --    with the "custom-pp" extension.
      ...
      liftIO $ putStrLn $ unlines $
        "package-with-build-tools: hs-custompp modules:"
        : ( map ( \ m -> "  - " ++ show m ) ppMods )
